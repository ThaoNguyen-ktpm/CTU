...
<div class="col">
<div class="container">
  <h2 class=" text-weight">Thêm Khóa Học<small></small></h2>
  <form  method="post" class="needs-validation KhoaHoc-form" action="/KhoaHoc/add" novalidate>
  @csrf
    <div class="group">
    <label>Tên Khóa Học <span style="color:red;">(*)</span></label>
      <input  id="usernameInput" name="TenKhoaHoc" type="text" class="form-control" required>
    ...
    <div class="group">
    <label>Chứng Chỉ <span style="color:red;">(*)</span></label>
    <select name="MaChungChi" class="form-control" required>
        ...
        <option value="{{$ChungChi1->id}}">{{$ChungChi1->TenChungChi}}</option>
        @endforeach
    </select>
    <div class="valid-feedback">
        Nhập Chứng Chỉ Thành Công
      </div>
    <div class="invalid-feedback">
        Vui Lòng Chọn Chứng Chỉ!
    </div>
    </div>
      <button name="Add" type="submit" class="submit-btn">Thêm Khóa Học</button>
  </form>
</div>
</div>
<div id="toast1"></div>

...
@section('content')
        ...
        <a href=" /VaiTro/addview"  aria-expanded="false" >
        ...
        <script>
            ...
            var table = $('#myTableVaiTro').DataTable({
                ajax: {
                    url: "{{ route('VaiTro.data') }}",
                    dataSrc: 'data'
                },
                columns: [
                    { data: 'id' },
                    { data: 'TenVaiTro' },
                    {
                        ...
                        return '<form method="get" action="/VaiTro/updateview/'+row.id+'">@csrf <button class="btn btn-success"  type="submit"><i class="fa-solid fa-pen-to-square" style="color: #ffffff;margin:0"></i></button></form>';
                    }
                    },
                    {
                        data: null,
                        render: function(data, type, row) {
                            return '<button class="btn btn-danger DeleteVaiTro-form" onclick="deleteVaiTro(' + row.id + ')"><i class="fa-solid fa-trash-can" style="color: #ffffff;margin:0"></i></button>';
                        }
                    }
                ]
            });
        ...
        function deleteVaiTro(VaiTroId) {
            if (confirm('Bạn có chắc chắn muốn xóa vai trò này?')) {
                // Gửi yêu cầu xóa vai trò đến server
                $.ajax({
                    url: '/VaiTro/remove/' + VaiTroId,
                    type: 'GET',
        ...
        $(document).ready(function() {
	$('.DeleteVaiTro-form').click(function(event) {
